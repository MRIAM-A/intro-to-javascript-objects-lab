...
[
  ...
  game.gyms.forEach(gyms => {
    console.log(`Gym completed: ${gyms.completed}`)
  });
...
let starterpokemonIndex = game.party.findIndex(pokemon => pokemon.starter === true);

if (starterpokemonIndex !== -1) {
    let starterpokemon = game.party[starterpokemonIndex];

let evolvedNumber;
switch (starterpokemon.number){
    case 1:
        evolvedNumber = 2;
        break;
    case 4:
...
if (evolvedpokemon){
    ...
    console.log("Evolved pokemon not found in the data.");
}
}
...
*/
pokemon.filter(p => p.starter === true). forEach(p =>{
    console.log(p.name);
});
...
game.catchpokemon(pokemon.find(p => p.number === 1));
console.log(game.party);
